...
for (cY = 1:imY); 
    ...
    for (cX = 1:imX);          
        ...
        thisPixelData = squeeze(double(im(cY,cX,:)));%RGB
...
%like = 3 by 1
[k ~] = size(data);
like = 1/(sqrt((2*pi)^k*det(gaussCov)))...
       *exp(-0.5*(data-gaussMean)' * gaussCov^(-1) * (data - gaussMean));
end
...
%TO DO (a): replace this
    meanData = mean(data');
    meanData = meanData';
    covData = (data*data')./nData - meanData*meanData';
end
