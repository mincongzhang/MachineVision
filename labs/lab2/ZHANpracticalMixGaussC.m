...
for (cData =1:nData)
    ...
    data(:,cData) = mixGauss.mean(:,h) + R'*randn(2,1);   
end;
...
for (cIter = 1:nIter)
   ...
   for (cData = 1:nData)
        ...
        thisXData = data(:,cData);
        for(j = 1:mixGaussEst.k)
          postHidden(j,cData) = mixGaussEst.weight(j)*getGaussProb(thisXData,mixGaussEst.mean(:,j),mixGaussEst.cov(:,:,j));
        end
...
for(cData = 1:nData)
    ...
    like = 0;
    for(j = 1:mixGaussEst.k)
    %getMultiGauss() is a function for calculating probablity of 
    %multivariate normal distribution, please go into the function to see
    %more details
    like = like + mixGaussEst.weight(j)*getGaussProb(data(:,cData),mixGaussEst.mean(:,j),mixGaussEst.cov(:,:,j));
    end
...
k = length(x);
prob = 1./((2*pi)^k*det(var))^0.5*exp(-0.5*(x-mean)'*(var^(-1))*(x-mean));