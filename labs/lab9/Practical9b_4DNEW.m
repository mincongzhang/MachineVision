...
for( iTime = 1:22 )
    ...
    for (particleNum = 1:numParticles)
        ...
        % motion model.
        noise = normrnd(0,5,[1,2])*0;
        particles_new(particleNum,1:2) = particles_old( samples_to_propagate(particleNum),1:2 ) +noise + particles_old( samples_to_propagate(particleNum),3:4 );
        particles_new(particleNum,3:4) = particles_old( samples_to_propagate(particleNum),3:4 ) + 3*randn(1,2);
