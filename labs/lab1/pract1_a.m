...
%derive the log or something


muML = sum(X)/length(X);    % So put your answer here in place of zero.
sigmaML = sqrt(    sum((X - muML).^2)/length(X)    ); % So put your answer here in place of zero.
...
% same maximum
lfun = zeros(length(sigmaRange), length(muRange));%likelihood function
llfun = zeros(length(sigmaRange), length(muRange));%log likelihood function
