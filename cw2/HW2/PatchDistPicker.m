...
%
%   imgMaskCel is a binary mask that lets you specify that only some pixels
%   in T should be evaluated, otherwise we'll use the whole rectangular
%   patch. Only relevant for DistName == masked_*
%
...
switch (DistName)
    ...
    case {'plain_nssd'}  % NOTE!! Negating since this gives _distance_ and we want _closeness_
        ...
        % FIX! Replacing line below to avoid problems with C having 4
        % identical elements, i.e. no max.
        %[movedSE scoreMaxs] = CompRelMotionVecsWoPadding( C, availPadWN );
        movedSE = [0 0]; scoreMaxs = C(1,1);
